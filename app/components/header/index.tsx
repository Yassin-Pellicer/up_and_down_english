...
export function Header() {
  ...
  const navItems = [
    ...
    { key: "resources", icon: "library_books" },
  ...
  return (
    <>
      <header className={`${isScrolled ? "bg-black/20 backdrop-blur-lg" : "bg-transparent"} 
        flex flex-row justify-between gap-x-4 py-4 xl:h-24 h-fit z-[50] text-white w-full px-8 transition-all duration-300 fixed top-0`}>
        <img
          src="/logo.png"
        ...
        <div className="fixed top-0 left-0 w-full h-screen z-[100] bg-black/50 backdrop-blur-sm" onClick={() => setMenuOverlay(false)}>
          <div className="flex flex-col gap-2 divide-y-2 divide-white/30  h-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-end gap-4 px-8 h-24 items-center">
              <span
                className="material-symbols-outlined hover:cursor-pointer text-white z-[1000] relative" onClick={() => setMenuOverlay(!menuOverlay)} style={{ fontSize: '2rem' }}>
                {menuOverlay ? 'close' : 'menu'}
              </span>
            </div>
            {navItems.map(({ key, icon }) => (
              <button key={key} className="flex px-6 py-3 cursor-pointer select-none btn btn-primary gap-2 items-center text-2xl text-white">
                <span className="material-symbols-outlined">{icon}</span>
                {t(`header.nav.${key}`)}
              </button>
            ))}
            <div className="flex px-4 flex-row gap-4 items-center mt-4">
              <span onClick={() => i18n.changeLanguage(i18n.language === 'en' ? 'pl' : 'en')}
                className="material-symbols-outlined hover:cursor-pointer text-white" style={{ fontSize: '2rem' }}>
                {t("header.translate")}
              </span>
            </div>
          </div>
